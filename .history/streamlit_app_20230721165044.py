...
if audio_data is not None:
    ...
    # voice_data, samplerate = sf.read(io.BytesIO(audio_data), frames=16000)[:, 0]
    voice_data, samplerate = sf.read(io.BytesIO(audio_data), frames=16000)
    
    st.write(samplerate)
    st.audio(voice_data, sample_rate=samplerate)
    st.write(voice_data.shape)
    st.write(voice_data[:, 0])
    
    are_equal = voice_data[:, 0] == voice_data[:, 1]

    num_equal_rows = np.count_nonzero(are_equal)
    num_not_equal_rows = len(are_equal) - num_equal_rows

    st.write("Number of rows that are equal:", num_equal_rows)
    st.write("Number of rows that are not equal:", num_not_equal_rows)
    ...
    if st.button('plots_1'):
        fig, axes = plt.subplots(2, figsize=(12, 8))
        timescale = np.arange(voice_data.shape[0])
        axes[0].plot(timescale, voice_data[:, 0])
        axes[0].set_title('Waveform')
        axes[0].set_xlim([0, 16000])
